...
export default function Home() {
  ...
  const handleStart = useCallback((mapId?: import('../game/types').MapId, difficulty?: import('../game/types').Difficulty) => {
  ...
  if (!gameState || !engineRef.current) {
    return (
      ...
      >
        <div style={{ color: '#fbbf24', fontSize: '1.25rem', fontFamily: "'Uncial Antiqua', serif" }}>
          Loading the realm...
        </div>
      </div>
    );
  ...
  if (isMenu) {
    return <MenuScreen onStart={(mapId, difficulty) => handleStart(mapId, difficulty)} />;
  }
  ...
  return (
    ...
    >
      ...
      <GameHUD state={gameState} engine={engineRef.current!} onPause={handlePause} onNextWave={handleNextWave} />
        ...
        <div style={{ position: 'relative', flex: 1, minWidth: 0, overflow: 'hidden' }}>
          ...
          {gs !== 'playing' && (
            <div style={{ position: 'absolute', inset: 0, zIndex: 10 }}>
              <GameOverlay state={gameState} onStart={handleStart} onRestart={handleRestart} />
            </div>
          )}
        </div>
